...
import copy
from envs.spacemouse.spacemouse_expert import SpaceMouseExpert
import requests
...
class SpacemouseIntervention(gym.ActionWrapper):
    def __init__(self, env, action_indices=None, device=None):
        super().__init__(env)
        ...
        self.expert = SpaceMouseExpert(device=device)
        
        self.left, self.right = False, False
        self.prev_left, self.prev_right = False, False
    ...
    def action(self, action: np.ndarray) -> np.ndarray:
        ...
        if left_pressed and not self.prev_left:
            print("[SpacemouseIntervention] CLOSE pressed")
        if right_pressed and not self.prev_right:
            print("[SpacemouseIntervention] OPEN pressed")
        ...
        if self.gripper_enabled:
            if left_pressed:  # close gripper
                gripper_action = np.random.uniform(-1, -0.9, size=(1,))
                intervened = True
            elif right_pressed:  # open gripper
