...
resolve_robotiq_comport() {
    local explicit="${LEFT_ROBOTIQ_COMPORT:-${LEFT_ROBOTIQ_PORT:-${FRANKA_ROBOTIQ_COMPORT:-${FRANKA_ROBOTIQ_PORT:-${ROBOTIQ_COMPORT:-${ROBOTIQ_PORT:-}}}}}}"
    local default_comport="${LEFT_ROBOTIQ_DEFAULT_COMPORT:-/dev/serial/by-id/usb-FTDI_USB_TO_RS-485_DAAQM2ZX-if00-port0}"

    if [[ -n "$explicit" ]]; then
        if [[ ! -e "$explicit" ]]; then
            echo "❌ 左臂 Robotiq 串口不存在：$explicit" >&2
            exit 1
        fi
        printf '%s\n' "$explicit"
        return 0
    ...
    echo "❌ 未能唯一确定左臂 Robotiq RS485 串口。" >&2
    echo "   默认串口不存在：$default_comport" >&2
    echo "   检测到 ${#rs485_ports[@]} 个 RS485 串口：" >&2
    printf '     %s\n' "${rs485_ports[@]}" >&2
    echo "   请设置 LEFT_ROBOTIQ_COMPORT 或 FRANKA_ROBOTIQ_COMPORT。" >&2
    exit 1
}

ROBOTIQ_COMPORT_RESOLVED="$(resolve_robotiq_comport)"
GRIPPER_SERVER_PORT="${LEFT_GRIPPER_SERVER_PORT:-50054}"
...
cd "$WORK_DIR"
launch_gripper.py \
    --config-name=launch_left_gripper \
