...
REPO_ID="Nahuyiur/clean_whiteboard"
LOCAL_DIR="/home/pnp/Desktop/clean_whiteboard"
LOG_DIR="/home/pnp/frankateleop/logs/modelscope_upload_clean_whiteboard"
STATE_DIR="/home/pnp/frankateleop/logs/modelscope_upload_clean_whiteboard/state"
...
while true; do
  ...
  attempt=$((attempt + 1))
  log_file="$LOG_DIR/clean_whiteboard_$(date +%Y%m%d_%H%M%S)_attempt${attempt}.log"
  ...
  stdbuf -oL -eL modelscope upload "$REPO_ID" "$LOCAL_DIR" \
    --repo-type dataset \
    --commit-message "Upload clean_whiteboard dataset" \
