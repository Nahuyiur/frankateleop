...
REPO_ID="${REPO_ID:-Nahuyiur/stack_cube}"
LOCAL_DIR="${LOCAL_DIR:-/home/pnp/Desktop/stack_cube}"
LOG_DIR="${LOG_DIR:-/home/pnp/frankateleop/logs/modelscope_upload_stack_cube}"
...
while true; do
  ...
  attempt=$((attempt + 1))
  log_file="$LOG_DIR/stack_cube_$(date +%Y%m%d_%H%M%S)_attempt${attempt}.log"
  ...
  stdbuf -oL -eL modelscope upload "$REPO_ID" "$LOCAL_DIR" \
    --repo-type dataset \
    --commit-message "Upload stack_cube dataset" \
