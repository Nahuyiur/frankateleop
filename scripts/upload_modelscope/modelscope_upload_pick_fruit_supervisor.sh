...
REPO_ID="${REPO_ID:-Nahuyiur/pick_fruit}"
LOCAL_DIR="${LOCAL_DIR:-/home/pnp/Desktop/pick_fruit}"
LOG_DIR="${LOG_DIR:-/home/pnp/frankateleop/logs/modelscope_upload_pick_fruit}"
...
while true; do
  ...
  attempt=$((attempt + 1))
  log_file="$LOG_DIR/pick_fruit_$(date +%Y%m%d_%H%M%S)_attempt${attempt}.log"
  ...
  stdbuf -oL -eL modelscope upload "$REPO_ID" "$LOCAL_DIR" \
    --repo-type dataset \
    --commit-message "Upload pick_fruit dataset" \
