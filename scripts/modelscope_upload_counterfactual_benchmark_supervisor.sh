...
REPO_ID="${REPO_ID:-Nahuyiur/counterfactual_benchmark}"
LOCAL_DIR="${LOCAL_DIR:-/home/pnp/Desktop/Counterfactual Benchmark}"
LOG_DIR="${LOG_DIR:-/home/pnp/frankateleop/logs/modelscope_upload_counterfactual_benchmark}"
...
if [[ ! -d "$LOCAL_DIR" ]]; then
  write_status "local directory not found: $LOCAL_DIR"
  exit 1
fi

attempt=0
write_status "supervisor started for ${REPO_ID}; local_dir=${LOCAL_DIR}; max_workers=${MAX_WORKERS}"
...
while true; do
  ...
  attempt=$((attempt + 1))
  log_file="$LOG_DIR/counterfactual_benchmark_$(date +%Y%m%d_%H%M%S)_attempt${attempt}.log"
  ...
  stdbuf -oL -eL modelscope upload "$REPO_ID" "$LOCAL_DIR" \
    --repo-type dataset \
    --commit-message "Upload counterfactual benchmark dataset" \
